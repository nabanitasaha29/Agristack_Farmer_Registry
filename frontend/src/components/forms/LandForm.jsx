...
const LandForm = forwardRef(
  ({ onSubmit, initialValues = {}, username }, ref) => {
    const [form] = Form.useForm();
    const [manualForm] = Form.useForm(); // Separate form instance for manual land addition
  ...
  useEffect(() => {
    ...
    const handleAddSelectedLands = () => {
      ...
      const landsToAdd = selectedLands.filter(
        (land) => !lands.some((addedLand) => addedLand.id === land.fr_land_id)
      );
    ...
    const handleAddLand = async (values) => {
      try {
        if (
          !manualFormLocation ||
          Object.keys(manualFormLocation).length === 0
        ) {
        ...
        message.error("Failed to add land. Please try again.");
      }
    };
    ...
    const generateTableColumns = () => {
      ...
      return [
        ...
        //namematch score
        {
          ...
          render: (score) => (
            <Tag
              color={score === 100 ? "green" : score > 50 ? "blue" : "orange"}
            >
              {`${score || 0}%`}
            </Tag>
          ),
    ...
    const getIdentifierColumns = (fieldMappings, displayIdentifiers) => {
      ...
      return Object.entries(identifierMappings).map(
    ...
    const identifierColumns = getIdentifierColumns(
      fieldMappings,
      landIdentifiers
    );

    const availableLandsColumns = useMemo(() => {
      if (!fieldMappings?.landIdentifiers || !fieldMappings?.location)
        return [];

      const { landId, area, areaUnit } = fieldMappings;
      ...
      return [
            ...
            <Checkbox
              ...
              onChange={(e) => {
                if (e.target.checked) {
                  ...
                  const landToAdd = availableLands.find(
                    (land) => land[landId] === id
                  );
                  setSelectedLands([...selectedLands, landToAdd]);
                } else {
                  setSelectedLands(
                    selectedLands.filter((land) => land[landId] !== id)
                  );
                }
              }}
        ...
        {
          title: `Area (${areaUnit})`,
          render: (record) => `${record[area]} ${record[areaUnit]}`,
        ...
        <div className="land-form-container">
          <Form form={form} layout="vertical">
            <div className="location-selection-section">
              <h3>Select Location to View Available Lands </h3>
              <LocationSelector
                form={form}
                ...
                onSelectionChange={(selected) => {
                  ...
                  locationHierarchy.forEach((level) => {
                    ...
                    if (matchKey && selected[matchKey]) {
                      newLocation[`level_${level.levelOrder}`] =
                        selected[matchKey];
                    }
                  });
              ...
              <div className="available-lands-section">
                ...
                {landsFetched &&
                  ...
                  availableLands.length === 0 && (
                    ...
                    >
                      No registered lands found at the selected location under
                      your name.
                    </div>
                  )}
          ...
          <div className="manual-land-section">
            ...
            >
              {showManualForm
                ? "Hide Manual Land Form"
                : "Add New Land Manually"}
            </Button>
            ...
            <Collapse activeKey={showManualForm ? ["1"] : []} ghost>
              <Panel key="1" header={null} showArrow={false}>
                <Form
                  ...
                  initialValues={{ fr_area_unit: areaUnit }}
                >
